...
def load_tests(system):
    ...
    alltests = pd.read_csv(
        '/home1/08402/vortebo/codebase/Initial-Condition-Solver-for-Tidal-Evolution-using-Probabilistic-Neural-Network/systests/'+str(system)+'_tests.csv'
    )
    keys=alltests.keys()
...
if __name__ == '__main__':
    logpath='/scratch/08402/vortebo/ls6/output/ML/training_output/'
    setup_process(
        fname_datetime_format='%Y%m%d%H%M%S',
        system='tbd',
        std_out_err_fname=logpath+'{task}_new/{system}_{now}_{pid:d}.outerr',
